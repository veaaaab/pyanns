...
struct SparseHNSW {
  int64_t nb;
  int32_t R, efConstruction;
  std::unique_ptr<HierarchicalNSW<SparseQuant>> hnsw = nullptr;
  ...
  void Build(const std::string &filename) {
    quant.add(filename);
    nb = quant.n;
    hnsw = std::make_unique<HierarchicalNSW<SparseQuant>>(quant, nb, R / 2,
                                                          efConstruction);
    std::atomic<int32_t> cnt{0};
    auto st = std::chrono::high_resolution_clock::now();
    hnsw->addPoint(0);
...
#pragma omp parallel for
  ...
  Graph<int32_t> GetGraph() { return std::move(final_graph); }
};
